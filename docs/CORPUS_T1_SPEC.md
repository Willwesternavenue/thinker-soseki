...
## 12.6 Phase C（小説コーパス）の投入（2026-07-28 進行中）
...
- [ ] 全9作品の Pass2 + embedding（バックグラウンド実行中）
- [ ] 品質レポート全項目
- [ ] snapshot 基準の更新（Phase A+C の新 digest）
...
原典にある話題と現代語の間がはっきり空くので、`MIN_DIRECT_SOURCE_SCORE = 0.45` を
その間に置いた。`hasDirectSource` と `directSourceIds` は**同じ条件**で数える。
