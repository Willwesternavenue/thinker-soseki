...
- **canonical_work**: 版をまたぐ作品の同一性。実データでは 113行 → **107作品**
...
| 000790 | 吾輩**ハ**猫**デアル** | わがはいはねこである | 旧字旧仮名 | **なし** |

判定は次の順で行い、**どの段でも決まらなければ人手確認キューへ**（自動統合しない）。

| 段 | 判定 | 備考 |
|---|---|---|
| 1 | **`作品名読み` の一致** | 上記の反例はここで統合できる。第一候補 |
| 2 | 正規化タイトルの一致 | NFKC + カタカナ→ひらがな + 長音・促音の正規化 |
...
- 原則 **新字新仮名** を `is_primary_retrieval_edition=true`（実データで84件が該当）
...
| 原則 | 本仕様での担保 |
|---|---|
| 一覧件数を独立作品数とみなさない | §1.1（113行 → 107作品）/ §1.2 の同定規則 |
| 作品と版を分ける | §1.1 の3層構造 |
