 ...
 *
 * ⚠️ **2026-08-09 に 0.45 → 0.40 へ変更。** 上の較正は483チャンク時点のもので、
 * 10,152チャンクでは分布が変わっていた。実測:
 *
 *   | 質問 | 最高スコア | 原典の被覆 |
 *   |---|---|---|
 ...
 *
 * **被覆あり(0.445)と被覆なし(0.443)は 0.002 差で、閾値では分離できない。**
 * その判別は `decideAbstention` の主題語の実在検査が担う。したがってこの閾値の
 * 役割は「明らかに無関係なものを落とす」ことだけに縮小しており、0.329 と 0.443 の
 * 間にあればよい。
 *
 * ⚠️ **この較正は実測3点に合わせたもので、根拠として弱い。** コーパスを増やしたら
 * 測り直すこと。0.45 のままだと明治維新のような**被覆のある質問が誤って留保に
 * 倒れる**（実際に倒れた）。
 */
export const MIN_DIRECT_SOURCE_SCORE = 0.4;
