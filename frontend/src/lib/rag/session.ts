...
/** 本人(執行草舟)を指す正規名。x_shigyo固有。 */
const SUBJECT_CANONICAL = "執行草舟";

/**
 * 検索クエリ内の「本人」を指す各種呼称を正規名「執行草舟」に寄せる(x_shigyo固有)。
 *
 * 「社長」は敬称であり、書き起こしには聞き手が本人を「社長」と呼ぶ箇所が大量にある。
 * 呼称のまま埋め込むと、質問内容ではなく敬称トークンにヒットして雑音(「質問者: 社長 /
 * 本人発言: おお」等)が上位に来る。本人を指す呼称を正規名に統一し、敬称由来の余分な
 * 一致を防ぐ。※検索クエリ(分類・ルーティング・原典検索)にのみ適用。回答生成は原文を使う。
 *
 * 対応: 執行社長 / 執行さん・氏・先生 / しぎょうさん・しぎょうそうしゅう /
 *       草舟・草舟さん等 / 社長(副社長は除外) / ご自身 / あなた(様) → 執行草舟
 */
export function normalizeSubjectReferences(query: string): string {
  return query
    // さん・敬称付きの固有表現(単独の「社長」ルールより先に処理する)
    .replace(/執行草舟(さん|先生|氏)/g, SUBJECT_CANONICAL)
    .replace(/執行社長/g, SUBJECT_CANONICAL)
    .replace(/執行(さん|先生|氏)/g, SUBJECT_CANONICAL)
    .replace(/しぎょう(そうしゅう|さん)/g, SUBJECT_CANONICAL)
    // 「草舟」単独(「執行草舟」の一部は誤置換しない)
    .replace(/(?<!執行)草舟(さん|先生|氏)?/g, SUBJECT_CANONICAL)
    // 敬称「社長」→ 本人。副社長など複合語は除外
    .replace(/(?<!副)社長/g, SUBJECT_CANONICAL)
    // このアバターに向けられた「ご自身」= 本人
    .replace(/ご自身/g, SUBJECT_CANONICAL)
    // このアバターに向けられた二人称「あなた(様)」= 本人。
    // 「あなたの経歴を教えて」が人物アンカー無しで埋め込まれ、プロフィール・職歴チャンクに
    // 全く届かない実測(類似度0.41→0.49、PROF_001/造船営業/年表が上位化)に基づく
    .replace(/あなた(様|さま)?/g, SUBJECT_CANONICAL)
    // 置換で連続した「執行草舟執行草舟」を1つに畳む
    .replace(/執行草舟(?:\s*執行草舟)+/g, SUBJECT_CANONICAL);
}
