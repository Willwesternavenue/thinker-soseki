 ...
 *
 * ⚠️ **誤判定は「文脈が付く」だけでは終わらない。**
 * 連結される直前の質問は毎回違うので、**同じ質問なのに埋め込みが変わり、検索
 * スコアが動く**。2026-08-09 実測: 「明治維新は…**それとも**精神を後退させたので
 * しょうか」が「それ」に当たって連結され、同一文面の4回で 0.43 / 0.445 / 0.445 /
 * 0.47 と振れた。留保の閾値 0.45 をまたぐ位置だったため、**同じ質問が日によって
 * 留保したりしなかったりしうる**状態だった(埋め込みAPI自体の揺らぎは 1e-4 で、
 * 固定文面なら 0.4454〜0.4455 に収まることを実測で確認している)。
 * 語を足すときは、その語を含む**自立した質問**が無いかを必ず確かめること。
 */
const ANAPHORA_RE =
  /それ(?!とも)|そこ|そちら|そう(いう|いった|した)|その(?!他|もの|ため|後)|これ(?!から)|こちら|この(?!世|ため|後|方|辺)|あれ|あそこ|あちら|あの(?!世|方|頃|時)/;
