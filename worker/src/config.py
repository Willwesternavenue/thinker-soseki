...
SUPABASE_URL = os.environ.get(
    "SUPABASE_URL", "https://thinker-soseki-TBD.supabase.co"
)
...
# 1試行あたりのタイムアウト(秒)。SDK既定はタイムアウト無しに近く、実際に
# 1本のハングが worker のパイプライン全体を46分止めた(2026-08-01、検出層の
# 較正走行)。⚠️ リトライ1回ごとにこの秒数がかかり得るので、最悪ケースの総待ち時間は
# 概ね LLM_TIMEOUT_SECONDS × (LLM_MAX_RETRIES+1)。無限よりは遥かにましだが、
# 詰まった1本がジョブ全体を長時間止める余地は残る(将来の改善余地)。
