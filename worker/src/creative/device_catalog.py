...
# 装置の単発発火率は 8/10 だった。同一 outline に独立に k 回走らせ、検出の
# **和集合**を取ることで見逃し率を単発の 0.2 から k=3 で 0.8% まで下げる。
#
# 検出は現在**監視モード**（ゲートしない。§device_catalog.py の呼び出し元を
# 参照）なので、和集合が偽陽性を増やしても作り直しは発生しない。偽陽性の代償は
# 監視ログの精度低下のみで、偽陰性の代償（見逃した装置混入がログに残らない）
# より軽い。
#
# k=3・model=Sonnet(既定) は 2026-08-01 の A/B で確定した恒久値
# （CREATIVE_PIPELINE_DESIGN.md §8 参照）。Haiku への降格は速度で3〜7倍
# 有利だが、既知の陰性6件中3件で誤検出し（Sonnetは1件のみ）、精度が
# 用途に見合わなかったため採用しなかった。モデルを混合する多様化案も、
# 同じ理由（精度の低いメンバーが union の偽陽性を増やすだけ）で見送った。
